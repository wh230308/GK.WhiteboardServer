    ...
    public abstract class HIDDevice : Win32Usb, IDisposable
    {
		...
		private static string GetDevicePath(IntPtr hInfoSet, ref DeviceInterfaceData oInterface)
		{
            ...
            if (!SetupDiGetDeviceInterfaceDetail(hInfoSet, ref oInterface, IntPtr.Zero, 0, ref nRequiredSize, IntPtr.Zero))
            {
                ...
                //oDetail.Size = 8;	// hardcoded to 5! Sorry, but this works and trying more future proof versions by setting the size to the struct sizeof failed miserably. If you manage to sort it, mail me! Thx
                //oDetail.Size = (IntPtr.Size == 4) ? (4 + Marshal.SystemDefaultCharSize) : 8;
                oDetail.Size = (IntPtr.Size == 4) ? 5 : 8;
		...
		public static HIDDevice FindDevice(int nVid, int nPid, Type oType)
        {
            ...
            IntPtr hInfoSet = SetupDiGetClassDevs(ref gHid, IntPtr.Zero, IntPtr.Zero, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);	// this gets a list of all HID devices currently connected to the computer (InfoSet)
            try
            {
                ...
                int nIndex = 0;
                //bool result = SetupDiEnumDeviceInterfaces(hInfoSet, 0, ref gHid, (uint)nIndex, ref oInterface);
